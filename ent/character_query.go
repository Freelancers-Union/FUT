...
type CharacterQuery struct {
	...
	withOutfit  *OutfitQuery
	withRibbons *RibbonQuery
	withFKs     bool
...
// QueryOutfit chains the current query on the "outfit" edge.
func (cq *CharacterQuery) QueryOutfit() *OutfitQuery {
	query := (&OutfitClient{config: cq.config}).Query()
...
// QueryRibbons chains the current query on the "ribbons" edge.
func (cq *CharacterQuery) QueryRibbons() *RibbonQuery {
	query := (&RibbonClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CharacterQuery) Clone() *CharacterQuery {
	...
	return &CharacterQuery{
		...
		withOutfit:  cq.withOutfit.Clone(),
		withRibbons: cq.withRibbons.Clone(),
...
// WithOutfit tells the query-builder to eager-load the nodes that are connected to
// the "outfit" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CharacterQuery) WithOutfit(opts ...func(*OutfitQuery)) *CharacterQuery {
	query := (&OutfitClient{config: cq.config}).Query()
...
// WithRibbons tells the query-builder to eager-load the nodes that are connected to
// the "ribbons" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CharacterQuery) WithRibbons(opts ...func(*RibbonQuery)) *CharacterQuery {
	query := (&RibbonClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withRibbons = query
	return cq
}
...
//		GroupBy(character.FieldCharacterID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Character.Query().
//		Select(character.FieldCharacterID).
//		Scan(ctx, &v)
...
func (cq *CharacterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Character, error) {
	...
	if query := cq.withOutfit; query != nil {
		if err := cq.loadOutfit(ctx, query, nodes, nil,
			func(n *Character, e *Outfit) { n.Edges.Outfit = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withRibbons; query != nil {
		if err := cq.loadRibbons(ctx, query, nodes, nil,
			func(n *Character, e *Ribbon) { n.Edges.Ribbons = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CharacterQuery) loadOutfit(ctx context.Context, query *OutfitQuery, nodes []*Character, init func(*Character), assign func(*Character, *Outfit)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Character)
	for i := range nodes {
		if nodes[i].outfit_characters == nil {
			continue
		}
		fk := *nodes[i].outfit_characters
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "outfit_characters" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CharacterQuery) loadRibbons(ctx context.Context, query *RibbonQuery, nodes []*Character, init func(*Character), assign func(*Character, *Ribbon)) error {
	...
	query.withFKs = true
	query.Where(predicate.Ribbon(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(character.RibbonsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.character_ribbons
		if fk == nil {
			return fmt.Errorf(`foreign-key "character_ribbons" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "character_ribbons" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
